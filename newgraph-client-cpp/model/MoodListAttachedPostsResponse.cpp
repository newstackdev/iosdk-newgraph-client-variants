...
{
    {
        ...
        for( auto& item : val[utility::conversions::to_string_t("value")].as_array() )
        {
            if(item.is_null())
            {
                m_Value.push_back( std::shared_ptr<MoodCreateResponse_posts>(nullptr) );
            }
            else
            {
...
std::vector<std::shared_ptr<MoodCreateResponse_posts>>& MoodListAttachedPostsResponse::getValue()
{
    return m_Value;
}

void MoodListAttachedPostsResponse::setValue(std::vector<std::shared_ptr<MoodCreateResponse_posts>> value)
{
    m_Value = value;
    m_ValueIsSet = true;
}
